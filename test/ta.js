...
(async () => {
    ...
    await page.type('input[name="username"]', '440013578');
    await page.type('input[name="password"]', 'dwdw');
    await Promise.all([
    ...
    const courses = await page.evaluate(() => {
        function genMarks(average) {
            ...
            // Re-adjust sum in case last was clamped
            const actualAvg = Math.round((marks.reduce((a, b) => a + b, 0) / 4) * 10) / 10;
